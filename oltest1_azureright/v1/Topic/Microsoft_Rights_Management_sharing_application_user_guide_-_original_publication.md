...
-   任意の種類のファイルの保護の完全なサポートと、一般的に使用される種類のテキストおよびイメージ ファイル用の組み込みビューアーを追加します。
...
### <a name="BKMK_Unprotect"></a>ファイルからの保護の削除
...
ユーザーの一覧の電子メール アドレスを入力し、スライダーを使用してファイルのアクセス許可を選択して、[**適用**] をクリックします。
...
ユーザーが Windows 以外のデバイスで保護されたファイルを表示できるようにするには、[**すべてのデバイスで使用を許可する**] をクリックします。ユーザーは自分のデバイス用の [Microsoft Rights Management 共有アプリケーションをダウンロードする](http://go.microsoft.com/fwlink/?LinkId=303970)必要があります。
...
4.  [**ダウンロードの詳細**] ページで、**Microsoft Rights Management sharing application.zip** を選択して、[**ダウンロード**] をクリックします。

5.  必要に応じて、Microsoft File Transfer Manager をインストールし、Microsoft Rights Management 共有アプリケーションのダウンロード手順を完了します。

#### Microsoft Rights Management 共有アプリケーションでサポートされる保護のレベル
Microsoft Rights Management 共有アプリケーションは、次の表に示すように、2 つの異なるレベルの保護をサポートします。

||||
|-|-|-|
|保護の種類|ネイティブ|ジェネリック|
|説明|テキスト、イメージ、Microsoft Office (Word、Excel、PowerPoint) ファイル、.pdf ファイル、および AD RMS をサポートする他のアプリケーションのファイルの種類については、ネイティブな保護で、暗号化と権限の適用 (アクセス許可) の両方を含む強力なレベルの保護が提供されます。|すべての他のアプリケーションとファイルの種類については、一般的な保護で、.pfile を使用したファイルのカプセル化と、ユーザーがファイルを開くことを許可されているかどうかを確認する認証の両方を含むレベルの保護が提供されます。|
|保護|ファイルは完全に暗号化され、次の方法で保護が適用されます。<br /><br />-   保護されたコンテンツが表示される前には、電子メールを使用してファイルを受け取るか、ファイルまたは共有のアクセス権限によってそれに対するアクセス権を付与されるユーザーについて、認証が正しく行われる必要があります。<br />-   さらに、ファイルが保護されるときにコンテンツの所有者によって設定される使用権限およびポリシーは、コンテンツが IP ビューアーで表示される (保護されたテキストとイメージ ファイルの場合) か、関連付けられたアプリケーションで表示される (他のサポートされているすべてのファイルの種類) ときに、完全に適用されます。|ファイルの保護は次の方法で適用されます。<br /><br />-   保護されたコンテンツが表示される前に、ファイルを開くことが許可されているユーザーに対して認証が成功し、アクセス権が付与される必要があります。承認に失敗した場合、ファイルは開きません。<br />-   コンテンツの所有者によって設定された使用権限とポリシーが表示され、目的の使用ポリシーが、承認済みのユーザーに伝えられます。<br />-   承認されたユーザーがファイルを開き、アクセスする操作の監査ログが発生しますが、サポートしていないアプリケーションによって使用権限は適用されません。|
|ファイルの種類の既定値|これは、次のファイルの種類の既定の保護レベルです。<br /><br />-   テキストとイメージ ファイル<br />-   Microsoft Office (Word、Excel、PowerPoint) ファイル<br />-   Portable Document Format (.pdf)<br /><br />詳しくは、「サポートされるファイルの種類」をご覧ください。|これは、完全な保護を通じてサポートされない他のすべてのファイルの種類 (.vsdx、.rtf など) の既定の保護です。|

### <a name="BKMK_SupportFileTypes"></a>サポートされるファイルの種類
Microsoft Rights Management 共有アプリケーションでサポートされているファイルの種類を、次の表に示します。

|ファイル拡張子|説明|元のファイル拡張子|
|-----------|------|-------------|
|.ptxt|保護されるテキスト ファイル|.txt|
|.pxml|保護される XML ファイル|.xml|
|.pjpg|保護される JPG イメージ ファイル|.jpg|
|.pjpeg|保護される JPEG イメージ ファイル|.jpeg|
|.ppng|保護される PNG イメージ ファイル|.png|
|.ptiff|保護される TIFF イメージ ファイル|.tiff|
|.pbmp|保護される Windows ビットマップ ファイル|.bmp|
|.pgif|保護される GIF イメージ ファイル|.gif|
|.pgiff|保護される GIFF イメージ ファイル|.giff|
|.pjpe|保護される JPE イメージ ファイル|.jpe|
|.pjfif|保護される JFIF イメージ ファイル|.jfif|
|.pjif|保護される JIF イメージ ファイル|.jif|
次の表では、Microsoft Office 2013、Office 2010、Office 2007 によってサポートされるファイルの種類を示します。これには、2 つの保護の種類があります。MsoIrmProtector と OpcIrmProtector です。これらの保護の種類の詳細については、「[Microsoft Office File Format Protectors (Microsoft Office ファイル形式プロテクター)](http://archive.msdn.microsoft.com/OfficeProtectors)」をご覧ください。

|||
|-|-|
|MsoIrmProtector は、次のファイル種類をサポートします。<br /><br />-   doc<br />-   dot<br />-   xla<br />-   xls<br />-   xlt<br />-   pps<br />-   ppt|OpcIrmProtector は、次のファイル種類をサポートします。<br /><br />-   docm<br />-   docx<br />-   dotm<br />-   dotx<br />-   xlam<br />-   xlsb<br />-   xlsm<br />-   xlsx<br />-   xltm<br />-   xltx<br />-   xps<br />-   potm<br />-   potx<br />-   ppsx<br />-   ppsm<br />-   pptm<br />-   pptx<br />-   thmx|

### <a name="BKMK_ScriptedInstall"></a>Microsoft Rights Management 共有アプリケーションの自動デプロイ
Windows 版の RMS 共有アプリケーションはスクリプト化されたインストールをサポートするため、企業のデプロイに適しています。

##### RMS 共有アプリケーションを自動デプロイ用にダウンロードするには
...
RMS 共有アプリケーションのセットアップ パッケージは、さまざまなデプロイ シナリオをサポートし、次のものが含まれます。

|説明|展開シナリオ|
|------|----------|
|Microsoft Online サインイン アシスタント|次のものに必要です。<br /><br />-   Office 2010 および Windows Azure RMS|
|Office 用の修正プログラム (KB 2596501)|次のものに必要です。<br /><br />-   Office 2010 および Windows Azure RMS|
|暗号化モード 2 (KB 2627273) 用の修正プログラム|次のものに必要です。<br /><br />-   Office 2010 および Windows Azure RMS|
|AD RMS クライアントおよび RMS 共有アプリケーション|次のものに必要です。<br /><br />-   Office 2013 および Windows Azure RMS<br />-   Office 2010 および Windows Azure RMS<br />-   Office 2013 および Active Directory RMS<br />-   Office 2010 および Active Directory RMS<br />-   RMS 共有アプリケーションのアップグレード|
|リボン用 Office アドイン|次のものに必要です。<br /><br />-   Office 2013 および Windows Azure RMS<br />-   Office 2013 および Active Directory RMS<br />-   Office 2010 および Active Directory RMS<br />-   RMS 共有アプリケーションのアップグレード|
|Windows Azure Active Directory Rights Management 準備ツール|次のものに必要です。<br /><br />-   Office 2010 および Windows Azure RMS|
> [!NOTE]
> **Office 2010 および Windows Azure RMS** のシナリオでは、Windows Azure RMS を使用している場合、または Active Directory RMS を使用しているけれども Windows Azure RMS を使用している別の会社の人にドキュメントを安全に送信したい場合があります。
> 
> Windows Azure Active Directory Rights Management 準備ツールをインストールおよび実行して Office 2010 をサポートすると、次の 2 つのことが行われます。
> 
> -   RMS 共有アプリケーションをサポートするように、レジストリが編集されます。
> -   ユーザーが “ブートストラップ” されます。つまり、コンピューターは AD RMS サーバーまたは Windows Azure RMS にアクセスして、RMS を使用するためにコンピューターとユーザーに必要な証明書を取得します。

次の手順を使用して、これらのデプロイ シナリオ用に RMS 共有アプリケーションをデプロイするために必要なコマンドを識別します。

-   Office 2013 および Windows Azure RMS

-   Office 2010 および Windows Azure RMS

-   Office 2013 または Office 2010 と Active Directory RMS

-   RMS 共有アプリケーションのアップグレード

コマンドの例では、ダウンロードして抽出したファイルを **\\server5\apps\rms** を使用してクライアント コンピューターがアクセスできるネットワーク共有にコピーしてあること、およびクライアント コンピューターにはアプリケーション インストール ログ ファイルを格納する **C:\Log files** という名前のフォルダーが既にあることが、想定されています。各インストールでは、インストール ログ ファイルの名前はユーザーが選択しますが、ファイル拡張子は .log である必要があります。

> [!IMPORTANT]
> RMS 共有アプリケーションをデプロイする前に、以下の手順で必要なコマンドをパッケージ化する必要があります。これは、すべてのユーザーのコンピューター コンテキストに、ローカル管理者権限でインストールできるようにするためです。その後は、グループ ポリシーや System Center Configuration Manager などの標準的なアプリケーション デプロイ メカニズムを使用して、コンピューターにそのパッケージをデプロイできます。
> 
> ただし、Windows Azure Active Directory Rights Management 準備ツールは例外です。このツールは、コンピューターのユーザーごとに 1 回、正常にレジストリを編集できるように昇格した特権で実行する必要があります。これを実行するためには、コマンドの実行をユーザーに依頼する (たとえば、電子メール メッセージ内のリンクまたはヘルプ デスク ポータル上のリンク)、ログオン スクリプトに追加するなど、さまざまな方法があります。ユーザーにローカル管理者アカウントがないために runas コマンドを使用できない場合は、ユーザーが指定したルールに従ってコマンドを自動的に昇格できるデプロイ ツールがあります。

##### Office 2013 および Windows Azure RMS 用に RMS 共有アプリケーションをデプロイするには
...
5.  次のコマンドを使用して、Office アドインをインストールします。
